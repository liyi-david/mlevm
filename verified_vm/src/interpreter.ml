...
let rec preprocess (raw_ops: char ptr) (len: int): Evm.opcode list =
    ...
    if len == 0 then [] else begin
        ...
        | 0x0b -> Evm.SIGNEXTEND    :: (preprocess (raw_ops +@ 1) (len - 1))
        | 0x10 -> Evm.LT            :: (preprocess (raw_ops +@ 1) (len - 1))
        | 0x11 -> Evm.GT            :: (preprocess (raw_ops +@ 1) (len - 1))
        | 0x12 -> Evm.SLT           :: (preprocess (raw_ops +@ 1) (len - 1))
        | 0x13 -> Evm.SGT           :: (preprocess (raw_ops +@ 1) (len - 1))
        | 0x14 -> Evm.EQ        :: (preprocess (raw_ops +@ 1) (len - 1))
        | 0x15 -> Evm.ISZERO    :: (preprocess (raw_ops +@ 1) (len - 1))
        (*
        AND = 0x17,
        OR = 0x17,
        XOR = 0x18,
        ...
        CALLER = 0x33,
        *)
        | 0x34 -> Evm.CALLVALUE     :: (preprocess (raw_ops +@ 1) (len - 1))
        | 0x35 -> Evm.CALLDATALOAD  :: (preprocess (raw_ops +@ 1) (len - 1))
        (*
        CALLDATASIZE = 0x36,
        CALLDATACOPY = 0x37,
        CODESIZE = 0x38,
        ...
        *)
        ...
        | 0x51 -> Evm.MLOAD         :: (preprocess (raw_ops +@ 1) (len - 1))
        | 0x52 -> Evm.MSTORE        :: (preprocess (raw_ops +@ 1) (len - 1))
        | 0x53 -> Evm.MSTORE8       :: (preprocess (raw_ops +@ 1) (len - 1))
        | 0x54 -> Evm.SLOAD         :: (preprocess (raw_ops +@ 1) (len - 1))
        | 0x55 -> Evm.SSTORE        :: (preprocess (raw_ops +@ 1) (len - 1))
        | 0x56 -> Evm.JUMP          :: (preprocess (raw_ops +@ 1) (len - 1))
        | 0x57 -> Evm.JUMPI         :: (preprocess (raw_ops +@ 1) (len - 1))
        | 0x58 -> Evm.PC            :: (preprocess (raw_ops +@ 1) (len - 1))
        | 0x59 -> Evm.MSIZE         :: (preprocess (raw_ops +@ 1) (len - 1))
        | 0x5a -> Evm.GAS           :: (preprocess (raw_ops +@ 1) (len - 1))
        | 0x5b -> Evm.JUMPDEST      :: (preprocess (raw_ops +@ 1) (len - 1))
        (*
        LOG0 = 0xa0,
        LOG1 = 0xa1,
        LOG2 = 0xa2,
        ...
        CALLCODE = 0xf2,
        *)
        | 0xf3 -> Evm.RETURN        :: (preprocess (raw_ops +@ 1) (len - 1))
        (*
        DELEGATECALL = 0xf4,
        CREATE2 = 0xf5,
        REVERT = 0xfd,
        ...
        *)
        | _ -> begin
            ...
            if 0x60 <= curr && curr <= 0x7f then
                ...
                (Evm.DUP windex) :: (preprocess (raw_ops +@ 1) (len - 1))
            else if 0x90 <= curr && curr <= 0x9f then
                let windex = (curr - 0x90) + 1 in
                (Evm.SWAP windex) :: (preprocess (raw_ops +@ 1) (len - 1))
            else
